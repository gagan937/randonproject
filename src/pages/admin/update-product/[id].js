...
export const getStaticProps = async (context) => {
  try {
    const [productRows] = await getPool().query(`SELECT ${productSelect} FROM products WHERE id=? LIMIT 1`, [context.params.id]);
    const [categories] = await getPool().query(`SELECT ${categorySelect} FROM categories ORDER BY name ASC`);
    if (!productRows.length) return { notFound: true };
    return { props: { product: productRows[0], categories }, revalidate: 1 };
  } catch (err) {
    console.error(err);
